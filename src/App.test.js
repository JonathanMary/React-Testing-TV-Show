...
test("Renders loading when API call is empty", () => {
    mockFetchShow.mockResolvedValueOnce();
    render(<App />);
    expect(screen.getByText(/fetching data.../i)).toBeVisible();
});

test("API call renders", async () => {
    mockFetchShow.mockResolvedValueOnce(apiCall);
    render(<App />);

    await waitForElementToBeRemoved(() => screen.getByText(/fetching data.../i));
})
